...
public class EventSerializer implements Serializer<PayslipRequested> {
    ...
    @Override
    public byte[] serialize(final String topic, final PayslipRequested event) {
        try {
            if (event == null) {
                return null;
            }
            ...
            return jsonb.toJson(event, PayslipRequested.class).getBytes(StandardCharsets.UTF_8);
        } catch (Exception e) {
